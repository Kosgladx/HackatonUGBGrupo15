...
def adicionar_semwhats(connection, cursor, telefone):
    semwhats_exists = execute_query(cursor, "SELECT * FROM semwhats WHERE telefone = %s", (telefone,))
    if semwhats_exists:
        print(f"Telefone {telefone} já existe na lista de sem WhatsApp, falha no cadastro.")
        return
    execute_query(cursor, "INSERT INTO semwhats (telefone) VALUES (%s)", (telefone,))
    connection.commit()
    return
